...
| `name` | TEXT | `CHECK (char_length(btrim(name)) BETWEEN 2 AND 120)` — FR-002; persisted pre-trimmed by `trg_brands_trim_name` (see below) |
...
CREATE TRIGGER trg_brands_trim_name
  ...
  FOR EACH ROW
  EXECUTE FUNCTION brands_trim_name();  -- sets NEW.name = btrim(NEW.name)
```

Runs before the length `CHECK` and the uniqueness index below evaluate, so the
stored `name` never carries leading/trailing whitespace — matching FR-002's "after
trimming surrounding whitespace" as the actual persisted value, not just a
length-validation detail.
...
application-level pre-check. `lower(btrim(name))` normalizes both case *and*
surrounding whitespace, matching FR-003's "ignoring case and surrounding
whitespace" exactly — `lower(name)` alone would let "Acme Coffee" and
"  Acme Coffee  " coexist as two distinct brands, which the spec explicitly
forbids. (The `btrim()` here is belt-and-suspenders with the trigger above, not
load-bearing on its own — but the index stays correct even if the trigger is ever
bypassed some other way.) A violation surfaces as a Postgres unique-violation,
...
- **Lifecycle**: created/replaced by `POST /brands/{id}/logo`, removed by
  ...
  (FR-014); at most one live object per brand at a time (FR-010) — a
  same-format replacement overwrites the existing object in place (identical
  path); a format-changing replacement uploads to a new path first, then
  deletes the old one once the new object is confirmed stored — never the
  reverse order, and never an unconditional delete-old-path step, since that
  would delete the new object on a same-format replacement (see `research.md`
  Decision 3)
